...
def snat_s_geroya_v_invantar(item_id, tg_id, hero_id):
    sql_code = f'UPDATE items SET hero_id = NULL WHERE item_name = {item_id} AND tg_id = {tg_id} ' \
...
def buy_item_user(tg_id, item_id, price: int, count=1):
    ...
    if value:
        sql_code2 = f'UPDATE items SET count = {value[0]+count} WHERE tg_id = {tg_id} and item_name = {item_id}'
    else:
